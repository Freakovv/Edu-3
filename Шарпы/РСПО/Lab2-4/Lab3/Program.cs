class Program
{
    static void Main()
    {
        while (true)
        {
            ...
            DateTime orderDate = DateTime.Parse(Console.ReadLine() ?? DateTime.Now.ToString("yyyy-MM-dd"));
            ...
            while (applicableDiscounts.Count > 0)
            {
                Console.Clear();
                Console.WriteLine($"Текущая стоимость заказа: {finalAmount}");
                ...
                decimal discountRate = applicableDiscounts[choice].CalculateDiscount(new Order { TotalAmount = finalAmount });
